...
class CustomSwinTransformer(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(CustomSwinTransformer, self).__init__(**kwargs)
        self.swin_transformer = SwinTransformerTiny224(include_top=False)

    def call(self, inputs):
        inputs = tf.cast(inputs, dtype=tf.float32)  # Convert inputs to regular TensorFlow tensor
        print(inputs)
        return self.swin_transformer(inputs)
...
def init_model_SWIN(model_dir, image_shape, learning_rate):
    model_weights_path = expanduser('Weights/initial_weights.hdf5')
    start_from_epoch = 0
    ...
    # Test
    inputs = tf.keras.layers.Input(shape=image_shape + (1,), dtype='uint8')
    image_repeat = MyChannelRepeat(3)(inputs)
    swin_transformer_output = CustomSwinTransformer()(image_repeat)
    flattened_outputs = tf.keras.layers.Flatten()(swin_transformer_output)
    outputs = tf.keras.layers.Dense(1, activation='sigmoid')(flattened_outputs)
    model = tf.keras.models.Model(inputs=inputs, outputs=outputs)

    # Real
    # model = SwinTransformerTiny224()
    #inputs = tf.keras.layers.Input(shape=(224, 224, 3), dtype='uint8')
    #outputs = SwinTransformerTiny224(include_top=False)(inputs)
    #outputs = tf.keras.layers.Dense(100, activation='softmax')(inputs)
    #model = tf.keras.models.Model(inputs=inputs, outputs=outputs)

    optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate)
    loss = tf.keras.losses.BinaryCrossentropy(from_logits=False)
    metrics = [tf.keras.metrics.AUC(curve='ROC', name='AUC'), tf.keras.metrics.BinaryAccuracy(name='binary_accuracy'), tf.keras.metrics.SensitivityAtSpecificity(specificity=0.85, name='SensAtSpec')]
